...
public class ShopProvider extends ContentProvider {
    ...
    private Uri insertShop(Uri uri,ContentValues values) {
        ...
        Integer quantity = values.getAsInteger(ShopEntry.COLUMN_QUANTITY);
    ...
    private int updateShop(Uri uri,ContentValues values,String selection,String[] selectionArgs) {
        ...
        if (values.containsKey(ShopEntry.COLUMN_SUPPLIER_NAME)) {
            String supplier_name = values.getAsString(ShopEntry.COLUMN_SUPPLIER_NAME);
        ...
        if (values.containsKey(ShopEntry.COLUMN_SUPPLIER_PHONE_NUMBER)) {
            // Check that the weight is greater than or equal to 0 kg
            Integer number = values.getAsInteger(ShopEntry.COLUMN_SUPPLIER_PHONE_NUMBER);
            if (number != null && number < 0) {
                throw new IllegalArgumentException("Phone requires valid number");
            }
        }

        // check that the weight value is valid.
        if (values.containsKey(ShopEntry.COLUMN_QUANTITY)) {
            // Check that the weight is greater than or equal to 0 kg
            Integer quantity = values.getAsInteger(ShopEntry.COLUMN_QUANTITY);
            if (quantity != null && quantity < 0) {
                throw new IllegalArgumentException("Quantity requires valid number");
